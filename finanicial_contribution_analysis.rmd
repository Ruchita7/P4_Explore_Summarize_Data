...
df <-read.csv('P00000001-FL.csv',row.names = NULL)

```

```{r echo=FALSE, Load_first_last_name}

df$cand_id.new <- as.character(df$cand_id)
fl_contrib <- extract(df, cand_id.new, c("last_name","first_name"), "(.*),\\s*(\\S*)\\s*.*", fill="left",perl=TRUE)
fl_cand <- unique(fl_contrib$first_name)
```

```{r echo=FALSE, candidate_gender}

gender <- gender(as.character(fl_cand), c(1941, 1971),method = "ssa",countries = "United States")

fl_contrib_gender<-left_join(x=fl_contrib,y=gender,by=c("first_name"= "name"))
fl_contrib_gender$name <-NULL
fl_contrib_gender$proportion_male <-NULL
fl_contrib_gender$proportion_female <-NULL
fl_contrib_gender$year_min <-NULL
fl_contrib_gender$year_max <-NULL
